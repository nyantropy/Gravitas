...
`modules/model/domain/skin/` owns `GtsSkinBinding` and its CPU-only validation. The
...
`tests/model/domain/skin/GtsSkinBindingTest` links only the skin target. Release-active
