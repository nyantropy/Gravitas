...
int prepare_decoder(StreamingContext *sc) {
  for (int i = 0; i < sc->avfc->nb_streams; i++) {
    if (sc->avfc->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
      ...
      if (fill_stream_info(sc->video_avs, (const AVCodec **)&sc->video_avc, &sc->video_avcc)) {return -1;}
    } else if (sc->avfc->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
      sc->audio_avs = sc->avfc->streams[i];
      sc->audio_index = i;

      if (fill_stream_info(sc->audio_avs, (const AVCodec **)&sc->audio_avc, &sc->audio_avcc)) {return -1;}
    } else {
...
int prepare_video_encoder(StreamingContext *sc, AVCodecContext *decoder_ctx, AVRational input_framerate, StreamingParams sp) {
  ...
  //change this to impact output video bitrate
...
int main(int argc, char *argv[])
{
  ...
  // sp.copy_audio = 0;
  // sp.copy_video = 0;
  // sp.video_codec = "libxvid";
  // sp.codec_priv_key = "libxvid-params";
  // sp.codec_priv_value = "";
  // sp.audio_codec = "aac";
  // sp.output_extension = ".avi";
