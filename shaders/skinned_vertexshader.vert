...
layout(location = 7) in uint instanceObjectIndex;

layout(std430, column_major, set = 4, binding = 0) readonly buffer SkinPalette {
    mat4 matrices[];
} skinPalette;
...
void main() {
    ...
    mat4 skin = inWeights.x * skinPalette.matrices[inJoints.x]
              + inWeights.y * skinPalette.matrices[inJoints.y]
              + inWeights.z * skinPalette.matrices[inJoints.z]
              + inWeights.w * skinPalette.matrices[inJoints.w];
    mat4 worldFromBind = objectData.model * skin;
    vec4 worldPosition = worldFromBind * vec4(inPosition, 1.0);
    mat3 modelBasis = mat3(worldFromBind);
