...
namespace
{
    ...
    template <class Node> GtsSkeletonValidationResult validateNodes(const std::vector<Node>& nodes)
    {
        GtsSkeletonValidationResult result;
        if (nodes.empty())
            result.diagnostics.push_back(
                {"SKELETON_EMPTY", "A skeleton requires at least one evaluation node.", "nodes"});
        if (nodes.size() > std::numeric_limits<uint32_t>::max())
            result.diagnostics.push_back(
                {"SKELETON_NODE_COUNT", "Node count exceeds the 32-bit index contract.", "nodes"});
            ...
            else if (const auto* matrix = std::get_if<glm::mat4>(&node.defaultLocalTransform))
            {
                ...
                if (!finite)
                    error("SKELETON_MATRIX_NONFINITE", "Matrix elements must be finite.", "defaultLocalTransform");
                         ...
                         (*matrix)[3][3] != 1.0f)
                    error("SKELETON_MATRIX_NOT_AFFINE",
                          "Matrix bottom row must be [0, 0, 0, 1].",
                          "defaultLocalTransform");
            }
            else
                error("SKELETON_TRANSFORM_INVALID",
                      "Local transform must contain TRS or a matrix.",
                      "defaultLocalTransform");
        }
        return result;
    }
...
{
    return validateNodes(skeleton.nodes);
}

GtsSkeletonValidationResult validateGtsSkeletonCompatibility(const GtsSkeletonCompatibility& compatibility)
{
    return validateNodes(compatibility.nodes());
}
