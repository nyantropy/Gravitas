...
struct TransformAnimationComponent
{
    ...
    // the bitmask of TransformAnimationMode values
    uint32_t activeModes = 0;

    // rotation animation settings, if active
    glm::vec3 rotationEulerFactors = glm::vec3(0.0f, 1.0f, 0.0f);
    ...
    inline bool hasMode(TransformAnimationMode mode) const
    {
        return (activeModes & static_cast<uint32_t>(mode)) != 0;
    }

    inline void enableMode(TransformAnimationMode mode)
    {
        activeModes |= static_cast<uint32_t>(mode);
    }

    inline void disableMode(TransformAnimationMode mode)
    {
        activeModes &= ~static_cast<uint32_t>(mode);
    }
