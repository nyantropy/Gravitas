...
namespace gts::tools
{
    ...
    void EngineToolRuntime::registerInputBindings(InputBindingRegistry& input)
    {
        gts::input::defaults::tools::install(input);
    }

    void EngineToolRuntime::prepare(const EcsControllerContext& ctx)
    {
        impl->prepare(ctx);
    }
    void EngineToolRuntime::update(const EcsControllerContext& ctx)
    {
        impl->update(ctx);
    }
    uint32_t EngineToolRuntime::controllerSystemCount() const
    {
        return impl->controllerSystemCount();
    }
    void EngineToolRuntime::shutdown()
    {
        impl->shutdown();
    }

    void EngineToolRuntime::setStartupOptions(ToolStartupOptions options)
    {
        impl->setStartupOptions(std::move(options));
    }
} // namespace gts::tools
