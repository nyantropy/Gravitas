...
namespace gts::tools
{
    ...
    public:
        ...
        std::string_view title() const override { return "Particles"; }
        ...
        void build(EngineToolContext& ctx, UiHandle parent, BitmapFont* font) override
        {
            ...
            header = createText(ctx.ui, root, {0.014f, 0.006f, 0.340f, 0.030f}, font,
                                "PARTICLE EMITTER", color(0.95f, 0.98f, 1.0f), 0.016f);
            summary = createText(ctx.ui, root, {0.014f, 0.038f, 0.350f, 0.028f}, font,
                                 "NO EMITTER", color(0.66f, 0.74f, 0.80f), 0.0125f);

            addButtonRow(ctx.ui, font, 0.074f, {
            ...
            addButtonRow(ctx.ui, font, 0.118f, {
            ...
            float y = 0.176f;
            addSlider(ctx.ui, font, y, FloatField::EmissionRate, "RATE", 0.0f, 180.0f); y += 0.044f;
            addSlider(ctx.ui, font, y, FloatField::MaxParticles, "MAX", 8.0f, 512.0f, true); y += 0.044f;
            addSlider(ctx.ui, font, y, FloatField::Intensity, "INTENSITY", 0.0f, 2.5f); y += 0.044f;
            addSlider(ctx.ui, font, y, FloatField::LifetimeMin, "LIFE MIN", 0.05f, 6.0f); y += 0.044f;
            addSlider(ctx.ui, font, y, FloatField::LifetimeMax, "LIFE MAX", 0.05f, 8.0f); y += 0.044f;
            addSlider(ctx.ui, font, y, FloatField::VelocitySpread, "SPREAD", 0.0f, 2.0f); y += 0.044f;
            addSlider(ctx.ui, font, y, FloatField::Drag, "DRAG", 0.0f, 3.0f); y += 0.044f;
            addSlider(ctx.ui, font, y, FloatField::Softness, "SOFT", 0.0f, 240.0f); y += 0.044f;
            addSlider(ctx.ui, font, y, FloatField::Vortex, "VORTEX", -2.0f, 2.0f); y += 0.044f;
            addSlider(ctx.ui, font, y, FloatField::NoiseStrength, "NOISE", 0.0f, 1.0f); y += 0.050f;

            addSlider(ctx.ui, font, y, FloatField::TintR, "TINT R", 0.0f, 1.0f); y += 0.038f;
            addSlider(ctx.ui, font, y, FloatField::TintG, "TINT G", 0.0f, 1.0f); y += 0.038f;
            addSlider(ctx.ui, font, y, FloatField::TintB, "TINT B", 0.0f, 1.0f); y += 0.038f;
            addSlider(ctx.ui, font, y, FloatField::TintA, "TINT A", 0.0f, 1.0f); y += 0.045f;

            addSlider(ctx.ui, font, y, FloatField::SizeStart, "SIZE IN", 0.02f, 2.0f); y += 0.038f;
            addSlider(ctx.ui, font, y, FloatField::SizeEnd, "SIZE OUT", 0.02f, 2.0f); y += 0.038f;
            ...
            footer = createText(ctx.ui, root, {0.014f, 0.724f, 0.350f, 0.024f}, font,
                                "F6 TO HIDE", color(0.80f, 0.86f, 0.92f), 0.0125f);
        }
    ...
    private:
                          ...
                          const std::vector<std::pair<ButtonAction, std::string>>& rowButtons)
        {
            ...
            const float width = (0.362f - gap * static_cast<float>(rowButtons.size() - 1))
                / static_cast<float>(rowButtons.size());

            for (size_t i = 0; i < rowButtons.size(); ++i)
            {
                const float x = 0.014f + static_cast<float>(i) * (width + gap);
                buttons.push_back({
                    rowButtons[i].first,
                    createButton(ui, root, {x, y, width, 0.032f}, font, rowButtons[i].second, 0.0125f)
                });
            }
        }
                       ...
                       bool wholeNumber = false)
        {
            sliders.push_back({
                field,
                createSlider(ui, root, y, name, minValue, maxValue, wholeNumber, font)
            });
        }
                          ...
                          const UiInteractionResult& interaction)
        {
            for (const SliderBinding& binding : sliders)
            {
                ...
                writeField(emitter,
                           binding.field,
                           valueFromSliderPointer(ui, binding.slider, interaction.pointerX));
                return;
            }
        }
        ...
        static void reloadEmitter(ECSWorld& world,
                                  EngineToolStateComponent& state,
                                  ParticleEmitterComponent& emitter)
        {
        ...
        static void saveEmitter(EngineToolStateComponent& state,
                                const ParticleEmitterComponent& emitter)
        {
            ...
            state.status = gts::particles::saveParticleEffect(emitter.effectPath, emitter)
                ? "SAVED"
                : "SAVE FAILED";
        }
                           ...
                           ParticleEmitterComponent* emitter)
        {
            ...
            for (const SliderBinding& binding : sliders)
            {
                updateSlider(ctx.ui,
                             binding.slider,
                             readField(*emitter, binding.field),
            ...
            {
                const auto& runtime =
                    ctx.world.getComponent<ParticleEmitterRuntimeComponent>(state.selectedEntity);
                                       ...
                                       const ParticleEmitterComponent* emitter)
        {
            if (emitter == nullptr)
            {
                switch (action)
                {
                    case ButtonAction::PreviousEmitter: return "PREV";
                    case ButtonAction::NextEmitter: return "NEXT";
                    case ButtonAction::Reload: return "RELOAD";
                    case ButtonAction::Save: return "SAVE";
                    case ButtonAction::Restart: return "RESET";
                    case ButtonAction::ToggleEnabled: return "ON";
                    case ButtonAction::ToggleLocalSpace: return "LOCAL";
                    case ButtonAction::CycleShape: return "SHAPE";
                    case ButtonAction::CycleBlend: return "BLEND";
                }
            }

            switch (action)
            {
                case ButtonAction::ToggleEnabled: return emitter->enabled ? "ON" : "OFF";
                case ButtonAction::ToggleLocalSpace: return emitter->localSpace ? "LOCAL" : "WORLD";
                case ButtonAction::CycleShape: return shapeName(emitter->shape);
                case ButtonAction::CycleBlend: return blendName(emitter->blend);
                case ButtonAction::PreviousEmitter: return "PREV";
                case ButtonAction::NextEmitter: return "NEXT";
                case ButtonAction::Reload: return "RELOAD";
                case ButtonAction::Save: return "SAVE";
